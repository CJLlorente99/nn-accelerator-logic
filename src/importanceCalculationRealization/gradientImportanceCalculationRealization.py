...
for modelName in ['bnn/bnn_prunedBT12_100ep_4096npl_all']:
# for modelName in ['bnn/bnn_prunedBT10_100ep_4096npl', 'bnn/bnn_prunedBT12_100ep_4096npl']:
    ...
    model = BNNBinaryNeuralNetwork(connectionsToPrune)
    ...
    uniqueInput = np.unique(model.input0, axis=0)
    print(f'Original length {model.input0.shape[0]}, only unique length {uniqueInput.shape[0]}, number of sample {sampleSize}')
    ...
    # Pruned info files
    dfPrunedLayer = pd.read_csv(f'data/savedModels/{modelName}_prunedInfol0.csv')
    ...
    for neuron in range(model.valueSTE0.shape[1]):
        df[f'OUT{neuron:04d}'] = model.valueSTE0[:, neuron]
        df.replace({f'OUT{neuron:04d}': -1}, 0, inplace=True)

        aux = pruneAndDrop(df, dfPrunedLayer, f'N{neuron:04d}')

        createPLAFileABC(aux, f'./data/plas/{modelName}/ABC/layer0/N{neuron:04d}')
        createPLAFileEspresso(aux, f'./data/plas/{modelName}/ESPRESSO/layer0/N{neuron:04d}')
    ...
    for i in range(4):
        if not os.path.exists(f'./data/plas/{modelName}_10e-4/ESPRESSOOptimizedPerClass_0/layer{i}/'):
            os.makedirs(f'./data/plas/{modelName}_10e-4/ESPRESSOOptimizedPerClass_0/layer{i}/')
        if not os.path.exists(f'./data/plas/{modelName}_10e-4/ESPRESSOOptimizedPerClass_2/layer{i}/'):
    ...
    for neuron in range(model.valueSTE0.shape[1]):
        ...
        createPLAFileEspresso(aux, f'./data/plas/{modelName}/ESPRESSOOptimizedPerClass_0/layer0/N{neuron:04d}', conflictMode=0)
        createPLAFileEspresso(aux, f'./data/plas/{modelName}/ESPRESSOOptimizedPerClass_2/layer0/N{neuron:04d}', conflictMode=2)
        df.drop(f'OUT{neuron:04d}', inplace=True, axis=1)
