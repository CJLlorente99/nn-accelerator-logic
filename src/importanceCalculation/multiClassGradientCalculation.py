...
for i in range(sampleSize):
    ...
    model.newActivation = True
    for nClass in range(10): # 10 classes
        model.backwardClassIndicator = nClass
        model.zero_grad()
        pred = model(X)
        pred[0, nClass].backward()
        model.newActivation = False
...
model.saveGradients(f'data/multiClassGradients/gradientsSignBin100epochs{neuronPerLayer}npl', training_data.targets.tolist()[:sampleSize])
