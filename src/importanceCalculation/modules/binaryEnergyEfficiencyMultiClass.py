...
class BinaryNeuralNetwork(nn.Module):
	...
	def registerHooks(self):
		...
		# Initialize lists per class
		for nClass in range(10):
			self.gradientsSTE0[nClass] = []
			self.gradientsSTE1[nClass] = []
			self.gradientsSTE2[nClass] = []
			self.gradientsSTE3[nClass] = []
			self.gradientsIden[nClass] = []
	...
	def backward_hook_ste0(self, module, grad_input, grad_output):
		self.gradientsSTE0[self.backwardClassIndicator].append(grad_input[0].cpu().detach().numpy()[0])

	def backward_hook_ste1(self, module, grad_input, grad_output):
		self.gradientsSTE1[self.backwardClassIndicator].append(grad_input[0].cpu().detach().numpy()[0])

	def backward_hook_ste2(self, module, grad_input, grad_output):
		self.gradientsSTE2[self.backwardClassIndicator].append(grad_input[0].cpu().detach().numpy()[0])

	def backward_hook_ste3(self, module, grad_input, grad_output):
		self.gradientsSTE3[self.backwardClassIndicator].append(grad_input[0].cpu().detach().numpy()[0])
	...
	def listToArray(self, neuronPerLayer):
		...
		for nClass in range(10):
			self.gradientsSTE0[nClass] = np.array(self.gradientsSTE0[nClass])
			self.gradientsSTE1[nClass] = np.array(self.gradientsSTE1[nClass])
			self.gradientsSTE2[nClass] = np.array(self.gradientsSTE2[nClass])
			self.gradientsSTE3[nClass] = np.array(self.gradientsSTE3[nClass])
	...
	def computeImportance(self, neuronPerLayer):
		...
		for nClass in range(10):
			importanceSTE0[nClass] = np.abs(self.gradientsSTE0[nClass])
			print(f'Importance STE0 Class {nClass} calculated')
			importanceSTE1[nClass] = np.abs(self.gradientsSTE1[nClass])
			print(f'Importance STE1 Class {nClass} calculated')
			importanceSTE2[nClass] = np.abs(self.gradientsSTE2[nClass])
			print(f'Importance STE2 Class {nClass} calculated')
			importanceSTE3[nClass] = np.abs(self.gradientsSTE3[nClass])
			print(f'Importance STE3 Class {nClass} calculated')
	...
	def saveGradients(self, baseFilename: str, targets: list):
		columnsInLayer1 = [f'N{i}' for i in range(len(self.gradientsSTE0[0][0]))]
		columnsInLayer2 = [f'N{i}' for i in range(len(self.gradientsSTE1[0][0]))]
		columnsInLayer3 = [f'N{i}' for i in range(len(self.gradientsSTE2[0][0]))]
		columnsInLayer4 = [f'N{i}' for i in range(len(self.gradientsSTE3[0][0]))]
  
		for nClass in range(10):
			aux = pd.DataFrame(self.gradientsSTE0[nClass], columns=columnsInLayer1)
			aux['target'] = targets
			aux.to_feather(f'{baseFilename}STE0Class{nClass}')

			aux = pd.DataFrame(self.gradientsSTE1[nClass], columns=columnsInLayer2)
			aux['target'] = targets
			aux.to_feather(f'{baseFilename}STE1Class{nClass}')

			aux = pd.DataFrame(self.gradientsSTE2[nClass], columns=columnsInLayer3)
			aux['target'] = targets
			aux.to_feather(f'{baseFilename}STE2Class{nClass}')

			aux = pd.DataFrame(self.gradientsSTE3[nClass], columns=columnsInLayer4)
			aux['target'] = targets
			aux.to_feather(f'{baseFilename}STE3Class{nClass}')
