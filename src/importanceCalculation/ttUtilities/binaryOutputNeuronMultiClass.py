...
class BinaryOutputNeuron:
	...
	def giveImportance(self, importance: dict, nNeuron: int, targets, threshold=10e-4):
		"""
		...
		auxImportance = {}
		for n in importance:
			auxImportance[n] = importance[n][:, nNeuron]
   
		importance = auxImportance

		for i in range(len(importance[0])):
			nPerClass[targets[i]] += 1
			if importance[targets[i]][i] > threshold:
				aux = 0
				for j in range(len(importance)):
					aux += importance[j][i]
				'''
                if the rest of the importances have almost no importance, we append 1
                if the rest of the importances have importance, we append <1
                '''
				dictImportance[targets[i]].append(importance[targets[i]][i]/aux)

		for n in dictImportance:
			self.importancePerClass['class' + str(n)] = sum(dictImportance[n]) / nPerClass[n] * 100
			self.importance += sum(dictImportance[n]) / nPerClass[n] * 100
