...
for modelName in ['eeb/eeb_prunedBT12_100ep_100npl_all']:
    ...
    model = BinaryNeuralNetwork(connectionsToPrune)
    ...
    dupList.append(getIdxDuplicates(model.valueSTE3))
    uniqueSTE3 = np.unique(model.valueSTE3, axis=0)
    print(f'Original length {model.valueSTE3.shape[0]}, only unique length {uniqueSTE3.shape[0]}, number of sample {sampleSize}')
    ...
    for iImp in range(len(importanceList)):
        ...
        print(f'importance number {iImp} has {importanceList[iImp].sum().sum()} ({importanceList[iImp].sum().sum() / importanceList[iImp].size *100}) entries above threshold {threshold} out of {importanceList[iImp].size}')
    ...
    # Pruned info files
    dfPrunedLayer0 = pd.read_csv(f'data/savedModels/{modelName}_prunedInfol0.csv')
    ...
    for neuron in range(model.valueSTE0.shape[1]):
        ...
        aux = pruneAndDrop(df, dfPrunedLayer0, f'N{neuron:04d}')
    ...
    for neuron in range(model.valueSTE0.shape[1]):
        ...
        createPLAFileEspresso(aux, f'./data/plas/{modelName}/ESPRESSOOptimizedPerEntry_0/layer0/N{neuron:04d}', conflictMode=0)
        createPLAFileEspresso(aux, f'./data/plas/{modelName}/ESPRESSOOptimizedPerEntry_2/layer0/N{neuron:04d}', conflictMode=2)
        df.drop(f'OUT{neuron:04d}', inplace=True, axis=1)
