...
if irregularPrune:
    ...
    for layer in prunedConnections:
        ...
        df.to_csv(f'savedModels/bnn_prunedIrregular{inputsAfterIrregularPrune}_{epochs}ep_{neuronPerLayer}npl_prunedInfo{layer}.csv', index=False)

    metrics = '\n'.join(testReturn(test_dataloader, train_dataloader, model, criterion))
    print(metrics)
    with open(f'savedModels/bnn_prunedIrregular{inputsAfterIrregularPrune}_{epochs}ep_{neuronPerLayer}npl.txt', 'w') as f:
        f.write(metrics)
        f.write('\n')
    ...
    torch.save(model.state_dict(), f'savedModels/bnn_prunedIrregular{inputsAfterIrregularPrune}_{epochs}ep_{neuronPerLayer}npl')
else:
    # First layer
    ...
    df.to_csv(f'savedModels/bnn_prunedRegular{inputsAfterRegularPrune}_{epochs}ep_{neuronPerLayer}npl_prunedInfo{1}.csv', index=False)
    ...
    columnTags = [f'N{i:4d}' for i in range(len(weightMask))]
    df = pd.DataFrame(np.array(idxs).T, columns=columnTags)
    df.to_csv(f'savedModels/bnn_prunedRegular{inputsAfterRegularPrune}_{epochs}ep_{neuronPerLayer}npl_prunedInfo{2}.csv', index=False)
    ...
    columnTags = [f'N{i:4d}' for i in range(len(weightMask))]
    df = pd.DataFrame(np.array(idxs).T, columns=columnTags)
    df.to_csv(f'savedModels/bnn_prunedRegular{inputsAfterRegularPrune}_{epochs}ep_{neuronPerLayer}npl_prunedInfo{3}.csv', index=False)

    metrics = '\n'.join(testReturn(test_dataloader, train_dataloader, model, criterion))
    print(metrics)
    with open(f'savedModels/bnn_prunedRegular{inputsAfterRegularPrune}_{epochs}ep_{neuronPerLayer}npl.txt', 'w') as f:
        f.write(metrics)
        f.write('\n')
        f.write(f'epochs {epochs}\n')
        f.write(f'batch {batch_size}\n')
    ...
    torch.save(model.state_dict(), f'savedModels/bnn_prunedRegular{inputsAfterRegularPrune}_{epochs}ep_{neuronPerLayer}npl')
